...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- forms (front-end only, no backend configured) ---------- */
  document.querySelectorAll('form:not([data-gform])').forEach(form => {
  ...
  /* ---------- forms that submit invisibly to a Google Form ---------- */
  document.querySelectorAll('form[data-gform]').forEach(form => {
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      const actionUrl = form.getAttribute('data-gform');
      const formData = new FormData();
      form.querySelectorAll('[data-entry]').forEach(field => {
        formData.append(field.getAttribute('data-entry'), field.value);
      });
      fetch(actionUrl, { method: 'POST', mode: 'no-cors', body: formData }).catch(() => {});
      const note = form.querySelector('.form-success');
      if (note) note.style.display = 'block';
      form.reset();
    });
